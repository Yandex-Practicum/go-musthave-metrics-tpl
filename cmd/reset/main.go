...
func main() {
	dir := "."
	// Простейший разбор флага -dir без пакета flag, чтобы не усложнять.
	args := os.Args[1:]
...
// generateSource формирует содержимое файла reset.gen.go: заголовок,
// объявление пакета, блок импортов и методы Reset для каждого типа.
func generateSource(pkgTypes *types.Package, pkgName string, named []*types.Named, marked map[*types.TypeName]bool) ([]byte, error) {
	...
	fmt.Fprintln(&out, `// Code generated by "reset"; DO NOT EDIT.`)
	fmt.Fprintln(&out)
